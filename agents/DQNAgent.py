...
class DQNAgent:
    ...
    def __init__(self, agent_info, Network, chkpt_dir='.'):
        ...
        self.action_space = np.linspace(self.min_action, self.max_action, self.num_actions).round(1)
        ...
        self.q_eval = Network(self.lr, self.input_dims[0], self.num_actions, self.chkpt_dir).double()

        self.q_next = Network(self.lr, self.input_dims[0], self.num_actions, self.chkpt_dir).double()
    ...
    def choose_action(self, observation):
        ...
        if np.random.random() > self.epsilon:
            ...
            actions = self.q_eval.forward(T.tensor(state))
    ...
    def sample_memory(self):
        state_batch, action_batch, reward_batch, next_state_batch, mask_batch = self.memory.sample(self.batch_size)
    ...
    def agent_start(self, state):
        ...
        # In DQN, we control all Supply Air for each VAV the same
        # action = sat_setpoint, [sat_st + mixed temp[i]]
        self.last_action = action
        self.last_state = state
        return self.last_action

    def agent_step(self, reward, state):
        # last_action = sat_setpoint, [sat_st + mixed temp[i]]
